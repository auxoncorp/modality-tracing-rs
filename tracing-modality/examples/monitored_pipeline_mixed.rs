...
fn main() {
    ...
    if let Err(e) = ctrlc::set_handler(move || {
        ...
        tracing::error!(
            err = &e as &dyn Error,
            timestamp = NanosecondsSinceUnixEpoch::now_saturating().0,
...
impl NanosecondsSinceUnixEpoch {
    ...
    fn now_saturating() -> NanosecondsSinceUnixEpoch {
        match Self::now() {
            Ok(nanos) => nanos,
            // The system time predates the unix epoch.
            Err(TimeCrime::TimeRanBackwards) => NanosecondsSinceUnixEpoch(0),
            // The system time exceeds what we can track in a u64.
            Err(TimeCrime::TooFarInTheFuture) => NanosecondsSinceUnixEpoch(u64::MAX),
        }
    }
}
#[derive(Debug)]
...
mod producer {
    ...
    pub fn run_producer(
        ...
        for i in std::iter::repeat(0..u64::MAX).flatten() {
            ...
            if i % 5 == 0 {
                let _maybe_nonce: Option<i64> = Some(rng.gen());
                let maybe_nonce = None;
                send_heartbeat(&monitor_tx, Component::Producer, timeline_id, maybe_nonce);
            }
    ...
    fn send_measurement(
        ...
        let timestamp = NanosecondsSinceUnixEpoch::now_saturating();
            ...
            match timed_recv_result {
                Ok(msg) => {
                    ...
                    if last_heartbeat_tx.elapsed() > HEARTBEAT_INTERVAL {
                        let _maybe_nonce: Option<i64> = Some(rng.gen());
                        let maybe_nonce = None;
                        send_heartbeat(&monitor_tx, Component::Consumer, timeline_id, maybe_nonce);
        ...
        loop {
            ...
            match timed_recv_result {
                ...
                Err(RecvTimeoutError::Disconnected) => {
                    tracing::info!(
                        timestamp = NanosecondsSinceUnixEpoch::now_saturating().0,
                        "Detected all monitor transmission channels closed."
                    );
                    tracing::info!(
                        timestamp = NanosecondsSinceUnixEpoch::now_saturating().0,
                        "Shutting down"
...
mod tests {
    ...
    fn can_make_a_timestamp() {
        let _x = NanosecondsSinceUnixEpoch::now().expect("Time crime");
        let _y = NanosecondsSinceUnixEpoch::now_saturating();
    }
}
