use tracing_modality::{TracingModality, timeline_id, TimelineId};
use rand::{thread_rng, Rng};
use std::thread::{sleep, JoinHandle, Builder};
...
fn main() {
    ...
    let mut channels: Vec<(usize, Sender<Message>, Receiver<Message>)> = (0..THREADS).map(|i| {
        let (tx, rx) = channel();
        (i, tx, rx)
    }).collect();
